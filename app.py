...
def normalize_phone(p: str) -> str:
    """
    TR odaklı normalize (kanonik çıktı: +905xxxxxxxxx)
    """
    if not p:
        return ""
...
def get_top_category() -> tuple[str, int]:
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("""
...
st.markdown("""
<style>
.block-container { padding-top: 1.1rem; padding-bottom: 2.5rem; max-width: 760px; }
...
with tab_query:
    ...
    if number_id:
        ...
        if not row:
            ...
            # ✅ Kategori güncelleme geri geldi
            card_start()
            ...
            # ✅ Şikayet ekleme geri geldi (B1 + A + D)
            card_start()
...
# -------------------- TAB: Admin (PIN + Dashboard + Filtreler + CSV) --------------------
with tab_admin:
    if not st.session_state.get("is_admin", False):
        ...
        # ✅ CSV’den önce seçenekler (C) geri geldi
